...
`gh`, `gcloud`, `cargo`, and most XDG-compliant tools write to
`~/.cache/<tool>` by default. In sandboxed shells where `~/.cache` is
restricted (read-only, missing, or a sentinel file), these calls fail with a
permission or "file exists" error before doing any real work.
...
successive invocations stay separated and the override does not touch the
...
**How to apply:** Set the variable inline for the single command, not as a
shell export — keeps the override scoped and visible in command history.
...
- Permission errors unrelated to the cache directory: redirection will not
  help; diagnose the actual error before reaching for `XDG_CACHE_HOME`.
